...
if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
    ...
    // Go up to admin directory if needed
    $login_url = $base_url . str_replace('/marine_requests.php', '', $current_dir) . '/login.php';
...
$currentPage = 'marine_requests.php';
...
try {
    // Fetch main request WITH ALL RELATED DATA including endorser/amendment requester info
    $query = "SELECT mr.*, 
                    u.username as user_company,
                    u.full_name as requester_name,
                    a.full_name as agent_name,
                    a.username as agent_username,
                    v.vessel_name,
                    endorser.username as endorser_name,
                    amend_requester.username as amendment_requester_name
            ...
            LEFT JOIN users endorser ON mr.endorsed_by = endorser.user_id
            LEFT JOIN users amend_requester ON mr.amendment_requested_by = amend_requester.user_id
            WHERE mr.marine_id = ?";
    $params = [$marine_id];
    ...
    if (!$marine_request) {
        echo "<p>Marine request not found.</p>";
        exit;
    }
    
    // Fetch crew details
    $crewStmt = $conn->prepare("SELECT * FROM marine_crew_details WHERE marine_id = ? ORDER BY id");
    $crewStmt->execute([$marine_id]);
    $crew_members = $crewStmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Fetch other services
    $serviceStmt = $conn->prepare("SELECT * FROM marine_other_services WHERE marine_id = ?");
    $serviceStmt->execute([$marine_id]);
    $other_services = $serviceStmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Fetch fuel & water services
    $fuelStmt = $conn->prepare("SELECT * FROM marine_fuel_water_services WHERE marine_id = ?");
    $fuelStmt->execute([$marine_id]);
    $fuel_water_services = $fuelStmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Fetch general works
    $workStmt = $conn->prepare("SELECT * FROM marine_general_works WHERE marine_id = ?");
    $workStmt->execute([$marine_id]);
    $general_works = $workStmt->fetchAll(PDO::FETCH_ASSOC);
...
$bodNo = $marine_request['bod_no'] ?: 'N/A';
$assignedAgent = $marine_request['agent_name'] ?: ($marine_request['agent_username'] ?: 'Not Assigned');
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        .status-assign { background-color: #DBEAFE; color: #1E40AF; }
        ...
        .status-request-amendment { background-color: #FEE2E2; color: #991B1B; }
...
<body class="font-display bg-background-light dark:bg-background-dark overflow-hidden">
    <div class="relative flex h-screen w-full">
        ...
        if ($currentUser['role'] === 'admin') {
            $activeMenu = 'marine_requests.php';
        }
                ...
                <div class="mx-auto max-w-7xl">
                    ...
                    <div class="mb-4">
                        <nav class="text-sm text-gray-500 dark:text-gray-400">
                            <a href="marine_requests.php" class="hover:text-primary">Request List</a>
                            <span class="mx-2">/</span>
                            <a href="marine_requests.php" class="hover:text-primary">Marine</a>
                            <span class="mx-2">/</span>
                            <span class="text-[#212529] dark:text-gray-200 font-bold"><?php echo htmlspecialchars($bodNo); ?></span>
                        </nav>
                    </div>
                    ...
                    <div class="mb-8">
                        <h2 class="text-[#212529] dark:text-gray-100 text-4xl font-black leading-tight tracking-[-0.033em]">
                            Marine Request Details</h2>
                        <p class="text-gray-600 dark:text-gray-400 mt-2 text-base font-normal leading-normal">
                            Detailed information for BOD No: <span class="bod-number-highlight"><?php echo htmlspecialchars($bodNo); ?></span>
                        </p>
                        
                        <!-- Status Pill - Large style -->
                        <div class="mt-4 flex items-center gap-4 flex-wrap">
                            <?php
                            $statusClass = 'status-' . str_replace(' ', '-', strtolower($marine_request['status']));
                            ?>
                            <span class="status-pill status-pill-lg <?php echo $statusClass; ?>">
                                <?php echo htmlspecialchars(strtoupper($marine_request['status'])); ?>
                            </span>
                            
                            <!-- Berth Assigned At section when status is "ASSIGNED" -->
                            ...
                            <!-- Optional: Show berth ID if available -->
                        ...
                        <!-- Display Endorsement/Amendment Information if status is endorsed or request amendment -->
                        <?php if ($marine_request['status'] === 'endorsed' && $marine_request['endorsed_by']): ?>
                        <div class="mt-4 flex flex-wrap gap-4">
                            <div class="flex items-center gap-3 bg-green-50 text-green-800 px-4 py-2.5 rounded-xl border border-green-200 shadow-sm">
                                <div class="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center">
                                    <span class="material-symbols-outlined text-green-600">check_circle</span>
                                </div>
                                <div>
                                    <p class="text-sm font-medium">Endorsed by</p>
                                    <p class="text-sm font-bold"><?php echo htmlspecialchars($marine_request['endorser_name'] ?: 'User ID: ' . $marine_request['endorsed_by']); ?></p>
                                    <p class="text-xs text-green-600 mt-0.5">
                                        <?php echo $marine_request['endorsed_at'] ? date('d/m/Y H:i', strtotime($marine_request['endorsed_at'])) : ''; ?>
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <?php if ($marine_request['status'] === 'request amendment' && $marine_request['amendment_requested_by']): ?>
                        <div class="mt-4 flex flex-wrap gap-4">
                            <div class="flex items-center gap-3 bg-red-50 text-red-800 px-4 py-2.5 rounded-xl border border-red-200 shadow-sm">
                                <div class="w-8 h-8 rounded-full bg-red-100 flex items-center justify-center">
                                    <span class="material-symbols-outlined text-red-600">edit_note</span>
                                </div>
                                <div>
                                    <p class="text-sm font-medium">Amendment requested by</p>
                                    <p class="text-sm font-bold"><?php echo htmlspecialchars($marine_request['amendment_requester_name'] ?: 'User ID: ' . $marine_request['amendment_requested_by']); ?></p>
                                    <p class="text-xs text-red-600 mt-0.5">
                                        <?php echo $marine_request['amendment_requested_at'] ? date('d/m/Y H:i', strtotime($marine_request['amendment_requested_at'])) : ''; ?>
                                    </p>
                                </div>
                            </div>
                            
                            <?php if (!empty($marine_request['amendment_notes'])): ?>
                            <div class="flex-1 bg-gray-50 rounded-xl p-4 border border-gray-200">
                                <p class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-2">Amendment Notes</p>
                                <p class="text-sm text-gray-800"><?php echo nl2br(htmlspecialchars($marine_request['amendment_notes'])); ?></p>
                            </div>
                            <?php endif; ?>
                        </div>
                        <?php endif; ?>
                    ...
                    <!-- Details Card - Enhanced with modern styling -->
                    <div class="w-full bg-white rounded-xl shadow-sm p-6 md:p-8 mb-8">
                        <div class="grid md:grid-cols-2 gap-x-12 gap-y-8">
                                    ...
                                    <div class="space-y-4">
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            ...
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo htmlspecialchars($marine_request['log_no'] ?: 'N/A'); ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            ...
                                            <span class="text-sm font-bold text-gray-900 col-span-2">
                                                <?php echo htmlspecialchars($bodNo); ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Company</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo htmlspecialchars($marine_request['company'] ?: 'N/A'); ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Vessel Name</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo htmlspecialchars($marine_request['vessel_name']); ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">PO Number</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo htmlspecialchars($marine_request['po_number'] ?: 'N/A'); ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                        ...
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            ...
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo htmlspecialchars($assignedAgent); ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Request By</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo htmlspecialchars($marine_request['requester_name'] ?: $marine_request['user_company'] ?: 'N/A'); ?>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <section class="marine-detail-section">
                                ...
                                <div class="bg-white rounded-xl p-6 shadow-lg border border-gray-200 min-h-[340px] flex flex-col">
                                    <div class="space-y-4">
                                        <?php
                                        // Format dates
                                        $eta = $marine_request['eta'] ? new DateTime($marine_request['eta']) : null;
                                        $etd = $marine_request['etd'] ? new DateTime($marine_request['etd']) : null;
                                        $actual_eta = $marine_request['actual_eta'] ? new DateTime($marine_request['actual_eta']) : null;
                                        $actual_etd = $marine_request['actual_etd'] ? new DateTime($marine_request['actual_etd']) : null;
                                        ?>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Est. Arrival (ETA)</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo $eta ? $eta->format('d/m/Y H:i') : 'N/A'; ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Est. Departure (ETD)</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo $etd ? $etd->format('d/m/Y H:i') : 'N/A'; ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Actual Arrival (ETA)</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo $actual_eta ? $actual_eta->format('d/m/Y H:i') : 'N/A'; ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Actual Departure (ETD)</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo $actual_etd ? $actual_etd->format('d/m/Y H:i') : 'N/A'; ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Created At</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo date('d/m/Y H:i', strtotime($marine_request['created_at'])); ?>
                                            </span>
                                        </div>
                                        <div class="grid grid-cols-3 gap-4 items-center">
                                            <span class="text-sm text-gray-600 col-span-1">Updated At</span>
                                            <span class="text-sm font-semibold text-gray-900 col-span-2">
                                                <?php echo date('d/m/Y H:i', strtotime($marine_request['updated_at'])); ?>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="md:col-span-2 marine-detail-section">
                                ...
                                <div class="bg-white rounded-xl p-6 shadow-lg border border-gray-200">
                                    <p class="text-gray-700">
                                        <?php echo htmlspecialchars($marine_request['remarks'] ?: 'No remarks'); ?>
                                    </p>
                                </div>
                            </div>
                                    ...
                                    <div class="flex items-center justify-between mb-4">
                                        ...
                                        <div class="flex items-center gap-3">
                                            <span class="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                                                Total: <?php echo count($crew_members); ?> crew
                                            </span>
                                            <span class="px-3 py-1 rounded-full text-xs font-bold <?php echo $marine_request['crew_transfer_type'] === 'sign_on' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'; ?>">
                                                <?php echo htmlspecialchars($marine_request['crew_transfer_type'] === 'sign_on' ? 'SIGN ON' : 'SIGN OFF'); ?>
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <section class="md:col-span-2 marine-detail-section">
                                    <div class="flex items-center justify-between mb-4">
                                        ...
                                        <span class="text-xs font-medium text-gray-500 bg-gradient-to-r from-amber-100 to-orange-100 px-3 py-1 rounded-full border border-amber-200 shadow-sm">
                                            <?php echo count($other_services); ?> item<?php echo count($other_services) > 1 ? 's' : ''; ?>
                                        </span>
                                    </div>
                            ...
                            <?php if (!empty($fuel_water_services)): ?>
                                <section class="md:col-span-2 marine-detail-section">
                                    <div class="flex items-center justify-between mb-4">
                                        ...
                                        <span class="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                                            <?php echo count($fuel_water_services); ?> item<?php echo count($fuel_water_services) > 1 ? 's' : ''; ?>
                                        </span>
                                    </div>
                                    ...
                                    <div class="border border-gray-200 rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 bg-gradient-to-r from-white to-gray-50">
                                        <table class="w-full">
                                            ...
                                            <tbody class="divide-y divide-gray-200">
                                                <?php foreach ($fuel_water_services as $index => $service): ?>
                                                    <?php 
                            ...
                            <?php if (!empty($general_works)): ?>
                                <section class="md:col-span-2 marine-detail-section">
                                    <div class="flex items-center justify-between mb-4">
                                        <h3 class="text-lg font-semibold text-gray-900">General Works</h3>
                                        <span class="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                                            <?php echo count($general_works); ?> item<?php echo count($general_works) > 1 ? 's' : ''; ?>
                                        </span>
                                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const requestId = '<?php echo $marine_id; ?>';
            let selectedFiles = [];
            ...
            successModalCloseBtn.addEventListener('click', () => {
                ...
                loadUploadedFiles(); // Refresh to show new files
            });

            // Load Uploaded Files
            async function loadUploadedFiles() {
                try {
                    const response = await fetch(`../api/get_uploaded_files.php?request_id=${requestId}`);
                    ...
                    <div class="flex items-center gap-3 overflow-hidden">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-xs text-gray-500">
                                By ${file.uploader_name || 'Admin'} • 
                                ${new Date(file.created_at).toLocaleDateString()}
                            </p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <a href="${file.file_path}" target="_blank" class="text-gray-400 hover:text-blue-600 p-2 rounded-full hover:bg-blue-50 transition-colors" title="View">
                            <span class="material-symbols-outlined">visibility</span>
                        </a>
                        <a href="${file.file_path}" download class="text-gray-400 hover:text-green-600 p-2 rounded-full hover:bg-green-50 transition-colors" title="Download">
