...
$currentPage = 'port_dues_requests.php';
?>
...
<head>
    ...
    <title>Port Dues Requests List</title>
                ...
                <div class="w-full">
                    ...
                    <div class="mb-4">
                        <nav class="text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <span class="text-[#212529] dark:text-gray-200">Port Dues Requests</span>
                        </nav>
                    </div>
                            ...
                            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="search"
                                        ...
                                        placeholder="Search Request No or Vessel Name..."
                    ...
                    <div class="table-container">
                        <table>
                            ...
                            <tbody>
                                <?php
                                // Build query with only port_dues requests
                                $whereConditions = ["request_type = 'port_dues'"];
                                $params = [];

                                if (!empty($_GET['search'])) {
                                    $searchTerm = '%' . $_GET['search'] . '%';
                                    $whereConditions[] = "(lightport_id LIKE ? OR vessel_name LIKE ? OR company_name LIKE ?)";
                                    $params[] = $searchTerm;
                                    $params[] = $searchTerm;
                                    $params[] = $searchTerm;
                                }
                                if (!empty($_GET['status_filter'])) {
                                    $whereConditions[] = "lpr.status = ?";
                                    $params[] = $_GET['status_filter'];
                                }
                                if (!empty($_GET['month'])) {
                                    $whereConditions[] = "MONTH(lpr.request_date) = ?";
                                    $params[] = $_GET['month'];
                                }
                                if (!empty($_GET['year'])) {
                                    $whereConditions[] = "YEAR(lpr.request_date) = ?";
                                    $params[] = $_GET['year'];
                                }
                                ...
                                // Count totals (create separate where clause without JOIN alias)
                                $countWhereConditions = [];
                                $countParams = [];

                                if (!empty($_GET['search'])) {
                                    $searchTerm = '%' . $_GET['search'] . '%';
                                    $countWhereConditions[] = "(lightport_id LIKE ? OR vessel_name LIKE ? OR company_name LIKE ?)";
                                    $countParams[] = $searchTerm;
                                    $countParams[] = $searchTerm;
                                    $countParams[] = $searchTerm;
                                }
                                $countWhereConditions[] = "request_type = 'port_dues'";
                                if (!empty($_GET['status_filter'])) {
                                    $countWhereConditions[] = "status = ?";
                                    $countParams[] = $_GET['status_filter'];
                                }
                                if (!empty($_GET['month'])) {
                                    $countWhereConditions[] = "MONTH(request_date) = ?";
                                    $countParams[] = $_GET['month'];
                                }
                                if (!empty($_GET['year'])) {
                                    $countWhereConditions[] = "YEAR(request_date) = ?";
                                    $countParams[] = $_GET['year'];
                                }

                                $countWhereClause = !empty($countWhereConditions) ? 'WHERE ' . implode(' AND ', $countWhereConditions) : '';
                                $countStmt = $conn->prepare("SELECT COUNT(*) FROM light_port_requests $countWhereClause");
                                $countStmt->execute($countParams);
                                $total_records = $countStmt->fetchColumn();
                                $total_pages = ceil($total_records / $results_per_page);

                                // Fetch data with user info (we don't need username anymore since we're showing company_name)
                                $sql = "SELECT lpr.lightport_id, lpr.vessel_name, lpr.company_name, 
                                               lpr.request_date, lpr.request_time, lpr.status
                                        FROM light_port_requests lpr 
                                        $whereClause 
                                        ORDER BY lpr.request_date DESC, lpr.request_time DESC 
                                ...
                                if (count($results) > 0) {
                                    ...
                                    foreach ($results as $row) {
                                        echo "<tr class='clickable-row' data-id='" . $row['lightport_id'] . "' style='cursor: pointer;'>";
                                        echo "<td>" . $no++ . "</td>";
                                        echo "<td>" . htmlspecialchars($row['lightport_id']) . "</td>";
                                    ...
                                    echo "<tr><td colspan='7' class='text-center py-4'>No port dues requests found.</td></tr>";
                                }
                                ?>
                            </tbody>
                        </table>
                    ...
                    <?php if ($total_pages > 1): ?>
                        <div
                            ...
                            <div class="flex flex-1 justify-between sm:hidden">
                                <?php if ($page > 1): ?>
                                    <a href="?page=<?php echo $page - 1; ?><?php echo !empty($_GET['search']) ? '&search=' . urlencode($_GET['search']) : ''; ?><?php echo !empty($_GET['status_filter']) ? '&status_filter=' . urlencode($_GET['status_filter']) : ''; ?><?php echo !empty($_GET['month']) ? '&month=' . urlencode($_GET['month']) : ''; ?><?php echo !empty($_GET['year']) ? '&year=' . urlencode($_GET['year']) : ''; ?>"
                                ...
                                <div>
                                    <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm"
                                        ...
                                        for ($i = 1; $i <= $total_pages; $i++) {
                                            ...
                                            if (!empty($_GET['status_filter'])) $urlParams .= '&status_filter=' . urlencode($_GET['status_filter']);
                                            if (!empty($_GET['month'])) $urlParams .= '&month=' . urlencode($_GET['month']);
                                            if (!empty($_GET['year'])) $urlParams .= '&year=' . urlencode($_GET['year']);
                                            
                                            echo "<a href='?page=$i$urlParams' class='relative inline-flex items-center px-4 py-2 text-sm font-semibold $active focus:z-20'>$i</a>";
                                        }
                                        ?>
                                    </nav>
                                </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            rows.forEach(row => {
                row.addEventListener('click', function () {
                    window.location.href = '../admin/port_dues_detail.php?id=' + this.dataset.id;
                });
            });
        });
    </script>
