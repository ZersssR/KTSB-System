<?php
// Protected port clearance edit page - requires authentication
...
// Fetch port clearance request details
try {
    $query = "SELECT pcr.*, u.full_name as request_by_name, u.username as request_by_username
              FROM port_clearance_requests pcr
              LEFT JOIN users u ON pcr.user_id = u.user_id
              WHERE pcr.clearance_id = ?";
    ...
    // Also fetch the current company from port_clearance_requests to ensure it's available
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $clearance_type = $_POST['clearance_type'] ?? 'inward';
        $receipt_no = $_POST['receipt_no'] ?? '';
        
        // Update query
        $updateQuery = "UPDATE port_clearance_requests SET 
                        vessel_name = ?, 
                        company_name = ?, 
                        request_date = ?, 
                        request_time = ?, 
        ...
        if ($success) {
            // Success message
            $successMessage = "Port clearance request updated successfully!";
            ...
            header("Refresh: 2; URL=portclearancedetail.php?id=" . $clearance_id);
        } else {
...
<head>
    ...
    <title>Edit Port Clearance Request</title>
                ...
                <div class="mx-auto max-w-4xl">
                    ...
                    <div class="mb-4">
                        <nav class="text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <span class="mx-2">/</span>
                            <a href="portclearancedetail.php?id=<?php echo htmlspecialchars($request['clearance_id'] ?? ''); ?>" 
                               class="hover:text-primary">Port Clearance Detail</a>
                            <span class="mx-2">/</span>
                    ...
                    <div class="mb-8">
                        <h2
                            ...
                            Edit Port Clearance Request</h2>
                    ...
                    <!-- Edit Form -->
                    <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) . '?clearance_id=' . $clearance_id; ?>" class="space-y-6">
                        <div class="bg-white rounded-xl shadow-sm p-6 md:p-8">
                            ...
                            <div class="grid md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" 
                                           value="<?php echo htmlspecialchars($request['clearance_id'] ?? ''); ?>" 
                                           class="form-input" 
                                           disabled>
                                </div>
                                        ...
                                        <select id="company_name" name="company_name" class="form-input" required>
                                            ...
                                            <?php foreach ($customers as $customer): ?>
                                                ...
                                                <option value="<?php echo htmlspecialchars($customer['company_name']); ?>"
                                                    ...
                                                    <?php if ($isTemporary) echo 'title="Temporary company from port_clearance_requests"'; ?>>
                                ...
                                <!-- Clearance Type (Editable Dropdown) -->
                                <div>
                                    <label for="clearance_type" class="form-label">Clearance Type *</label>
                                    <div class="select-wrapper">
                                        <select id="clearance_type" name="clearance_type" class="form-input" required>
                                            <option value="inward" <?php echo (($request['clearance_type'] ?? '') == 'inward') ? 'selected' : ''; ?>>
                                                Inward
                                            </option>
                                            <option value="outward" <?php echo (($request['clearance_type'] ?? '') == 'outward') ? 'selected' : ''; ?>>
                                                Outward
                                            </option>
                                        </select>
                                    </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm p-6 md:p-8">
                            ...
                            <!-- Receipt File Display (Non-editable) -->
                            <?php if (!empty($request['receipt_file'])): ?>
                                <div class="mt-6 pt-4 border-t border-gray-200">
                                    <h4 class="text-sm font-semibold text-gray-900 mb-2">Current Receipt File</h4>
                                        ...
                                        <div class="flex items-center gap-3 overflow-hidden">
                                            ...
                                            <div class="min-w-0">
                                                <p class="text-sm font-medium text-gray-900 truncate">
                                                    <?php echo basename($request['receipt_file']); ?>
                                                </p>
                                                <p class="text-xs text-gray-500">Initial receipt file</p>
                                            </div>
                                        </div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-2">
                                        Note: To change the receipt file, please upload a new file in the detail page.
                                    </p>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-3">
                            <button type="button" 
                                    onclick="window.location.href='port_clearance_requests.php'"
